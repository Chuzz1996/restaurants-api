...
export class UserRepository {
    ...
    async addRestaurant(userId: string, restaurant: string): Promise<number>{
        const result = await this.userModel.updateOne({ _id: userId },{ $push: {restaurant: restaurant }});
        return result.modifiedCount;
    }

    async removeRestaurante(userId: string, restaurant: string): Promise<number>{
        const result = await this.userModel.updateOne({_id: userId},{$pull: {restaurant: restaurant}});
        return result.modifiedCount;
    }
